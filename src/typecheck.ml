...
type ctype = iden option * (iden * atype) list * (iden * atype list) list (* interface type * storage types * (ident * params) list *)
[@@deriving show {with_path = false}]
...
let extract_contract a (id, im, fl, el) = 
  if List.mem id a.symbols then failwith ("Duplicate declaration for: " ^ id) else ();
...
| [] -> a
| Parse_tree.DContract (id, im, fl, el) :: pt' -> extract_types pt' @@ extract_contract a (id, im, fl, el)
